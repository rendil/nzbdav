...
public class RadarrClient : ArrClient
{
    ...
    public override async Task<bool> UnmonitorFileAsync(string filePath, CancellationToken ct = default)
    {
        try
        {
            ...
            var updatePayload = new
            {
                ...
                sortTitle = targetMovie.SortTitle,
                ...
                hasFile = targetMovie.HasFile,
                movieFile = targetMovie.MovieFile
            };
    ...
    public override async Task<bool> MonitorFileAsync(string filePath, CancellationToken ct = default)
    {
        try
        {
            ...
            if (movies == null)
            {
                Log.Warning("Failed to retrieve movies from Radarr instance '{InstanceName}' for monitoring", _instanceName);
                return false;
            }
            ...
            Log.Debug("Attempting to monitor movie '{Title}' (ID: {MovieId}) with payload: {Payload}",
                targetMovie.Title, targetMovie.Id, System.Text.Json.JsonSerializer.Serialize(updatePayload));
