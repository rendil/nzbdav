...
public class SonarrClient : ArrClient
{
    ...
    public override async Task<(bool Success, int? Id)> DeleteFileAsync(string filePath, CancellationToken ct = default)
    {
        try
        {
            ...
            var episodeId = episodes?.Select(e => e.Id).FirstOrDefault();
            ...
            if (success)
            {
                Log.Information("Successfully deleted episode file '{FilePath}' (ID: {FileId}) containing {EpisodeCount} episodes from Sonarr instance '{InstanceName}'",
                    filePath, targetEpisodeFile.Id, episodeId ?? 0, _instanceName);
    ...
    public override async Task<bool> MonitorFileAsync(string filePath, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                Log.Warning("Failed to retrieve episodes for episode file ID {EpisodeFileId} from Sonarr instance '{InstanceName}' for monitoring",
                    targetEpisodeFile.Id, _instanceName);
                return false;
            }

            var successCount = 0;

            // Monitor each episode individually
            foreach (var episode in episodes)
            {
                // Only monitor if currently unmonitored
                if (episode.Monitored)
                {
                    Log.Debug("Episode '{Title}' S{Season:D2}E{Episode:D2} is already monitored in Sonarr instance '{InstanceName}'",
    ...
    public override async Task<bool> UnmonitorFileAsync(string filePath, CancellationToken ct = default)
    {
        try
        {
            ...
            if (episodes == null || episodes.Length == 0)
            {
                Log.Warning("Failed to retrieve episodes for episode file ID {EpisodeFileId} from Sonarr instance '{InstanceName}' for unmonitoring",
                    targetEpisodeFile.Id, _instanceName);
                return false;
            }
            ...
            // Unmonitor each episode individually
            foreach (var episode in episodes)
            {
                // Only unmonitor if currently monitored
                if (!episode.Monitored)
                {
                    Log.Debug("Episode '{Title}' S{Season:D2}E{Episode:D2} is already unmonitored in Sonarr instance '{InstanceName}'",
                        episode.Title, episode.SeasonNumber, episode.EpisodeNumber, _instanceName);
                    successCount++;
                    continue;
                }
