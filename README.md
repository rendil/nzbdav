...
- 🗂️ **FUSE Filesystem** - _Mount content as a local filesystem using FUSE_
- 🌐 **NFS Sharing** - _Share content over the network using NFS_
...
In addition to WebDAV + RClone, NZB-Dav now supports direct filesystem access via FUSE and network sharing via NFS:

## FUSE Filesystem

Mount NZB-Dav content directly as a local filesystem:

```bash
# Enable FUSE in your environment
FUSE_ENABLED=true
FUSE_MOUNT_POINT=/mnt/nzbwebdav

# Content becomes available at:
/mnt/nzbwebdav/
├── nzbs/                    # Raw NZB files
...
Share content over the network using the lightweight NFS server:

```bash
# Start with NFS support
docker-compose -f docker-compose.yml -f docker-compose.override.yml up

# Mount on remote clients
sudo mount -t nfs -o vers=4,ro server-ip:/nzbwebdav /mnt/remote-nzbdav
```

For detailed setup instructions, see [FUSE_NFS_SETUP.md](FUSE_NFS_SETUP.md).

# Radarr / Sonarr

Once you have the content accessible on your filesystem (via WebDAV+RClone, FUSE, or NFS), you can configure NZB-Dav as your download-client within Radarr and Sonarr, using the SABnzbd-compatible api.
